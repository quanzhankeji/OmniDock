...
final class FinderCommandCoordinatorTests: XCTestCase {
    ...
    func testCutAnnouncesBothTheEmptiedAndTheFilledFolder() throws {
        let root = try makeTemporaryDirectory()
        let source = root.appendingPathComponent("Source", isDirectory: true)
        let destination = root.appendingPathComponent("Destination", isDirectory: true)
        for url in [source, destination] {
        ...
        FinderFileCommandCoordinator(
            ...
            revealFiles: { _ in },
            requestDirectoryAccess: { _ in nil },
            announceDirectoryChange: { announced.append($0) }
        ).handle(requestID: request.id)

        // Revealing the pasted file refreshes the folder it landed in, but a
        // window showing the folder it left needs telling too, or it keeps
        // listing an item that is no longer there.
        XCTAssertTrue(announced.contains(source.standardizedFileURL))
        XCTAssertTrue(announced.contains(destination.standardizedFileURL))
    }

    func testCopyAnnouncesOnlyTheFolderThatGainedTheItem() throws {
        let root = try makeTemporaryDirectory()
        let source = root.appendingPathComponent("Source", isDirectory: true)
        let destination = root.appendingPathComponent("Destination", isDirectory: true)
        for url in [source, destination] {
        ...
        XCTAssertEqual(announced, [destination.standardizedFileURL])
    }
