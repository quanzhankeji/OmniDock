...
- 可在 Finder 右键菜单中加入复制路径、新建空白文件、显示或隐藏隐藏文件，以及用指定应用打开所选项目；没有选择项目时则打开当前文件夹。
- 可启用本机剪贴板历史，通过 `Command-Shift-C` 搜索文字、富文本、链接、图片和文件。
- 将不常用的菜单栏图标收纳到可展开的分界线后方，支持手动展开和自动隐藏。
- 可在 macOS 13 或更高版本中设置 OmniDock 登录后自动启动。
...
启用后，在 Finder Sync 可监控的本地文件夹空白处点击右键，可使用**复制路径**、可配置的**新建文件**子菜单、显示或隐藏隐藏文件的命令，以及用指定应用打开当前文件夹的可选快捷操作。默认包含文本和 Markdown，也可以在 OmniDock 设置中添加其他文件类型。右键点击所选项目时，可使用**复制路径**以及用指定应用打开所选项目的可选快捷操作；隐藏文件命令只显示在文件夹空白处菜单中。复制多个项目时，每条路径会单独占一行；新文件使用可用的 `NewFile.<extension>` 名称，不会覆盖已有文件，并会直接进入重命名状态。

OmniDock 会注册本地的 Desktop、Documents、Downloads、存在时的 iCloud Drive 对应目录，以及你明确授权的其他本地文件夹。由文件提供方管理的位置最终是否显示菜单取决于 Finder，因此部分云端文件夹可能不会显示这些命令。
