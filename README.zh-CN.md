...
Finder Sync 菜单仅适用于 macOS 允许第三方 Finder Sync 扩展监控的位置。由 File Provider 管理的位置不会显示这些菜单，包括由 iCloud Drive 管理的 Desktop 和 Documents 文件夹。
...
临时、保密和自动生成的剪贴板条目会被忽略。历史数量可设置为 1–999 条，并可在设置中删除单条记录或清空全部历史。
