...
Entry contents are encrypted on disk. OmniDock seals the copied content, its preview text, its thumbnail, and the source app name with AES-256-GCM, using a random key generated on first use and kept in the macOS login Keychain; the deduplication fingerprint is a keyed value that does not reveal what was copied. The copy time, entry kind, size, and copy count remain readable so the archive can be listed and pruned. If the key cannot be read from the Keychain, OmniDock keeps that session's history in memory only rather than writing it unprotected. History that was sealed with a key the current install no longer has, for example after restoring a backup without its Keychain item, cannot be recovered and is discarded.
